      ...
      children: [
        Expanded(
          child: BlocBuilder<FetchDrugsCubit, FetchDrugsState>(
            builder: (BuildContext context, FetchDrugsState state) {
              if (state is FetchDrugsSuccess) {
                final List<DrugModel> filteredDrugs = query.isEmpty
                    ? state.drugs
                    : state.drugs
                        .where((DrugModel drug) => drug.nameEn
                            .toLowerCase()
                            .contains(query.toLowerCase()))
                        .toList();
                if (myDrugs.isNotEmpty) {
                  isAddedList = filteredDrugs
                      .map((DrugModel drug) => myDrugs.any((DrugModel d) => d.id == drug.id))
                      .toList();
                } else {
